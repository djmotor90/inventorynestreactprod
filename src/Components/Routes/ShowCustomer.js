...
function ShowCustomer (){
    ...
    const checkForToast = () => {
        if (searchParams.get('purchaseSuccess')){
            setToastMessage('You have successfully simulated a customer delivery!')
            setToastShow(true);
        }
        else if (searchParams.get('addSuccess')){
            setToastMessage('You have successfully added a new customer!')
            setToastShow(true);
        }
        else if (searchParams.get('editSuccess')){
            setToastMessage('You have successfully edited your customer!')
            setToastShow(true);
        }
    };
    ...
    <div>
        ...
        <Container fluid>
            ...
            <Card data-bs-theme="dark">
                <Card.Header as="h5">{name} has made {tableLength} Purchase(s) </Card.Header>
                <Card.Body>
            ...
            <Row style={{padding:'10px 40px'}}>
                <Card  data-bs-theme="dark">
                    <Card.Header as="h5"> Simulate a Delivery request as {name} </Card.Header>
                    {displaydeliveryForm}
                </Card>
            </Row>
        </Container>
