...
function EditCustomer (){
    ...
    const makeAPICall = async () => {
        try {
          const response = await fetch(`http://132.145.219.172:3001/customers/${id}/edit`, {mode:'cors'});
      ...
      const displayForm = formInputData  && <GeneralForm data = {formInputData} path= 'customers' type='edit'/>;
    return(
      <div className="newFormHolder">
          <Card border="info" data-bs-theme="dark" className="newFormHolder">
            <Card.Header><Card.Title style= {{color: '#61dafb'}}> Edit Your Customer's Information </Card.Title></Card.Header>
            <Card.Body>
                {displayForm}
                <Button variant="primary" onClick={() => handleReturnClick()}>Return to General Customer Information</Button>
            </Card.Body>
          </Card>
      </div>
    );
